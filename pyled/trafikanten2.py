...
STOPIDS[u'Heimdalsgata'] = 3010531

STOPS = {}
STOPS[STOPIDS[u'Heimdalsgata']] = 'E'
...
def compressName(name):
    ...
    name = name.replace(' skysstasjon', ' s.stasj')
...
def fetchRealTimeData(stopid=0, compressNames=True, fake=False):
    ...
    trafikantenUrl = 'http://reis.trafikanten.no/reisrest/realtime/getalldepartures/' + str(stopid)
    ...
    for bus in buses:
        ...
        bus['RecordedAtTime'] = time.gmtime() #dateStrToStruct(bus['RecordedAtTime'])
...
while True:
    ...
    try:
        buses = []
        for stopId in STOPS:
            print "Performing HTTP request for stopId=%s" % stopId, 
            print "<" + stopIdToName(stopId) + ">"
            # TODO fix: buses += filterByDirection(fetchRealTimeData(stopId), STOPS[stopId])
            buses += fetchRealTimeData(stopId)
            dumpBusList(buses)
